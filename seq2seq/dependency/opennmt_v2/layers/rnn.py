...
from dependency.opennmt_v2.layers import common
...
class RNN(tf.keras.layers.Layer):
  ...
  #def __call__(self, inputs, mask=None, training=None, initial_state=None):
    """Forwards the arguments the RNN layer.

    Returns:
      A tuple with the output sequences and the state.
    """
    #outputs = self.rnn(inputs, mask=mask, training=None, initial_state=initial_state)
    outputs = self.rnn(*args, **kwargs)
    if isinstance(self.rnn, tf.keras.layers.Bidirectional):
      sequences = outputs[0:2]
      states = outputs[2:]
      """to-do: why we have several tensors for state per direction"""
