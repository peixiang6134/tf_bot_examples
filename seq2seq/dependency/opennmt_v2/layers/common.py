...
from dependency.opennmt_v2.utils.misc import shape_list


@function.Defun(
    python_grad_func=lambda x, dy: tf.convert_to_tensor(value=dy),
...
def dropout(x, rate, training=None):
  ...
  if compat.is_tf2():
    ...
    return tf.nn.dropout(x, 1 - (1.0 - rate))
...
class LayerNorm(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.bias = self.add_variable(
        "beta", [depth], initializer=tf.compat.v1.keras.initializers.Constant(0))
    self.scale = self.add_variable(
        "gamma", [depth], initializer=tf.compat.v1.keras.initializers.Constant(1))
