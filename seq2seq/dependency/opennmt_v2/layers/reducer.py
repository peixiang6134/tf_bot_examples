...
def pad_in_time(x, padding_length):
  ...
  x = tf.pad(tensor=x, paddings=[[0, 0], [0, padding_length], [0, 0]])
  x.set_shape((None, None, depth))
  return x

def align_in_time(x, length):
  """Aligns the time dimension of :obj:`x` with :obj:`length`."""
  time_dim = tf.shape(input=x)[1]
  return tf.cond(
      pred=tf.less(time_dim, length),
...
def pad_with_identity(x, sequence_length, max_sequence_length, identity_values=0, maxlen=None):
  ...
  """
  if maxlen is None:
    maxlen = tf.reduce_max(input_tensor=max_sequence_length)
  ...
  x = pad_in_time(x, maxlen - tf.shape(input=x)[1])
...
def pad_n_with_identity(inputs, sequence_lengths, identity_values=0):
  ...
  Returns:
    ...
    sequence length.
  """
  max_sequence_length = tf.reduce_max(input_tensor=sequence_lengths, axis=0)
  maxlen = tf.reduce_max(input_tensor=[tf.shape(input=x)[1] for x in inputs])
  padded = [
      pad_with_identity(
...
class ConcatReducer(Reducer):
  ...
  def reduce_sequence(self, inputs, sequence_lengths):
    ...
    if axis == 2:
      ...
      maxlen = tf.reduce_max(input_tensor=combined_length)
